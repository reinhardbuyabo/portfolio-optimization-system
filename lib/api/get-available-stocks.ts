...
function loadSectorData(): Map<string, { name: string; sector: string }> {
  ...
  try {
    ...
    console.log(`Loaded sector data for ${sectorMap.size} stocks`);
  } catch (error) {
...
export function getAvailableStocks(): StockInfo[] {
  try {
    ...
    // Parse header to find Code and Name columns
    const headers = parseCSVLine(lines[0]);
    const codeIndex = headers.findIndex(h => h.toUpperCase() === 'CODE');
    const nameIndex = headers.findIndex(h => h.toUpperCase() === 'NAME');
    
    if (codeIndex === -1 || nameIndex === -1) {
      console.error('Could not find Code or Name columns in CSV');
      return [];
    }
    
    // Parse all rows to get unique stocks
    for (let i = 1; i < lines.length; i++) {
      const line = lines[i].trim();
      if (!line) continue;
      ...
      if (code && name && !stocksMap.has(code)) {
        // Get sector info if available
        const sectorInfo = sectorMap.get(code);
        
        stocksMap.set(code, {
          code,
          name: sectorInfo?.name || name, // Prefer sector CSV name
